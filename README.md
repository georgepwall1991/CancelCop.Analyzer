...
```xml
<PackageReference Include="CancelCop.Analyzer" Version="1.39.1">
...
```powershell
Install-Package CancelCop.Analyzer -Version 1.39.1
```
...
| **CC031** | Avoid blocking synchronization primitives (`ManualResetEventSlim.Wait`, `WaitHandle.WaitOne`, `Monitor.Wait`, `Thread.Join`, `ReaderWriterLockSlim.Enter*Lock`/`TryEnter*Lock`) in async code | Warning | ❌ |
...
> CC026, which can offer a real fix. `ReaderWriterLockSlim.Enter*Lock` / `TryEnter*Lock` are
> included because they are not `WaitHandle` members and would otherwise be a silent false
> negative. A zero-timeout `TryEnter` is an immediate probe and stays quiet.

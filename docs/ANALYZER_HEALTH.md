...
A deliberately harsh health audit for the fifty-three implemented CancelCop rule IDs (CC001–CC006, CC009–CC053).
...
| CC036 | Blocking `Socket` calls in async code | Usage | Warning | 4 | 4 | 4 | 4 | 4 | 4 | Low | **v1.38.0 (new):** flags blocking `Socket` operations in async code. Deliberately *not* folded into CC028: that rule can offer a fix only because it demands a signature-compatible counterpart, and Socket's async APIs are not shaped that way (`Receive(byte[])` → `ReceiveAsync(Memory<byte>, CancellationToken)`), so extending it would trade fix safety for reach. `NetworkStream` stays with CC028 via its `Stream` coverage. Socket resolved from the compilation and matched by symbol through the override chain; async-context gated. Analyzer-only — no mechanical rewrite exists. **v1.52.46 (fixer):** a rewrite is offered wherever a compiling TAP arity binds — `byte[]` implicitly converts to `Memory<byte>` for Receive/Send, flag-bearing sends bind `SendAsync(..., SocketFlags, ct)`, endpoint connects bind `ConnectAsync(EndPoint, ct)`; every candidate is proven by speculative rebind, spines hoist, await-unsafe contexts withheld. |
...
| CC050 | Blocking `Ping.Send` in async code | Usage | Warning | 4 | 4 | 4 | 4 | 4 | 4 | Low | **v1.52.38 (new):** flags `Ping.Send` in async code (every sync overload begins with a hostname or `IPAddress` argument — there is no parameterless form; later arguments add `int`/`TimeSpan` timeout, `byte[]` buffer, and options). CC036–CC044 are Socket/Tcp/Udp/HttpListener/named-pipe/DNS; Ping produced **zero** diagnostics from every shipped rule (empirical all-analyzers test). Symbol-gated to `System.Net.NetworkInformation.Ping`; quiet unless `SendPingAsync` exists — the event-based EAP `SendAsync` is deliberately not treated as the counterpart; look-alikes and sync methods stay quiet. The token-taking `SendPingAsync` overloads are modern .NET only and exist solely on the `TimeSpan` arity, so a bare `Send(host)` rewrites tokenless while the full-arity call flows an in-scope token. Null-conditional statements hoist to an `is not null` guard; await-forbidden contexts (lock bodies, unsafe) and a bare implicit-this `Send` inside a `SendPingAsync` member are reported without a fix. |
| CC051 | Blocking `SslStream.AuthenticateAsClient` in async code | Usage | Warning | 4 | 4 | 4 | 4 | 4 | 4 | Low | **v1.52.39 (new):** flags `SslStream.AuthenticateAsClient` in async code (string host; certificate-collection and `SslProtocols` arities). CC036–CC044 are Socket/Tcp/Udp/HttpListener/named-pipe/DNS; SslStream produced **zero** diagnostics from every shipped rule (empirical all-analyzers test). Symbol-gated to `System.Net.Security.SslStream`; not sealed, so overrides match through the `.OverriddenMethod` chain; quiet unless `AuthenticateAsClientAsync` exists — the APM `BeginAuthenticateAsClient`/`EndAuthenticateAsClient` pair is never treated as the counterpart; look-alikes and sync methods stay quiet. Only the `SslClientAuthenticationOptions` arity accepts a token, so string-arity rewrites are tokenless while an options-arity call flows the in-scope token. Null-conditional statements hoist to an `is not null` guard; lock bodies/unsafe contexts and a bare implicit-this call inside an `AuthenticateAsClientAsync` member are reported without a fix. |
| CC052 | Blocking `WebRequest.GetResponse` in async code | Usage | Warning | 4 | 4 | 4 | 4 | 4 | 4 | Low | **v1.52.40 (new):** flags `WebRequest.GetResponse` in async code (parameterless). CC036–CC044 are Socket/Tcp/Udp/HttpListener/named-pipe/DNS; the legacy WebRequest stack produced **zero** diagnostics from every shipped rule (empirical all-analyzers test). Symbol-gated to `System.Net.WebRequest`; both `GetResponse` and its TAP counterpart are virtual on the framework type, so derived types (`HttpWebRequest`, `FileWebRequest`) match through the `.OverriddenMethod` chain; quiet unless `GetResponseAsync` exists — the APM `BeginGetResponse`/`EndGetResponse` pair is never treated as the counterpart; look-alikes and sync methods stay quiet. The TAP is parameterless and tokenless, so the rewrite never invents a token. Null-conditional statements hoist to an `is not null` guard; lock bodies/unsafe contexts and a bare implicit-this call inside a `GetResponseAsync` member stay fix-free. |
| CC053 | Blocking `Thread.Join` in async code | Usage | Warning | 4 | 4 | n/a | 4 | 4 | 4 | Low | **v1.52.42 (new):** flags `Thread.Join` in async code — the parameterless, `int`-timeout, and `TimeSpan` arities. Verified against the net9/net10 ref packs: `System.Threading.Thread` declares only those three non-virtual overloads, declares **no** TAP `JoinAsync` counterpart on any shipped .NET, and is sealed, so no derived-type receiver shapes exist and CC053 is analyzer-only by design: every diagnostic is reported without a rewrite (fix strategy n/a), no code-fix provider is exported. Symbol-gated to `System.Threading.Thread`; look-alike classes, sync methods, and other member names stay quiet. `Thread.Join` moved out of CC031 (v1.52.42) so each join call reports exactly once. ||
...
| Low | All 53 rules | Mature and FP-clean. Every rule is covered by a clean-code FP guard (`AllAnalyzersCleanCodeTests`) spanning core, framework (controllers/MediatR/SignalR/Minimal API/BackgroundService/gRPC), nested-scope, exotic-syntax, modern-C#-shape, async-File-I/O, and non-async `using` cases. Improve opportunistically. |

The rule set has grown from the original 9 (CC001–CC006, CC009) to 53 (adding CC010–CC053 across the
...
- ~~**`WebRequest.GetResponse` still silent.**~~ Shipped as CC052 in v1.52.40.
...
- v1.52.51: 1587 tests, green locally. **Docs/tests audit** — package version synced to the release train; consolidated changelog for the docs/test-only interim tags (1.52.47–1.50); no behavior change.
...
- v1.52.42: 1570 tests, green locally. **CC053 (new)** —
  ...
  declares **no** TAP `JoinAsync` counterpart on any shipped .NET, and is
  sealed — so CC053 is analyzer-only by design: every diagnostic is reported
  without a rewrite and no code-fix provider is exported. Symbol-gated to
  `System.Threading.Thread`; look-alike classes, sync methods, and other
  member names stay quiet. `Thread.Join` moved out of CC031's family
  (v1.52.42) into this dedicated rule so each join call reports exactly once.
- v1.52.40: 1558 tests, green locally. **CC052 (new)** —
  flags blocking `WebRequest.GetResponse` in async code; fixer rewrites to
  `await GetResponseAsync`. Verified against the net9 ref pack: the only
  `GetResponseAsync` is a parameterless `Task<WebResponse>` — no arity in
  the family accepts a token — so every rewrite stays tokenless even with a
  token in scope; real cancellation means leaving the legacy stack.
  Virtual on both sides, so `HttpWebRequest`/`FileWebRequest` match through
  the `.OverriddenMethod` chain back to `WebRequest`. Null-conditional
  statements hoist to an `is not null` guard; lock bodies/unsafe contexts
  and a bare implicit-this call inside a `GetResponseAsync` member stay
  fix-free. The APM `BeginGetResponse`/`EndGetResponse` pair is never
  treated as the counterpart.
- v1.52.39: 1544 tests, green locally. **CC051 (new)** —
  ...
  calls rewrite tokenless while an options-arity call flows the token;
  null-conditional statements hoist to an `is not null` guard,
  lock bodies/unsafe contexts and a bare implicit-this call inside an
  `AuthenticateAsClientAsync` member stay fix-free. The APM
  `BeginAuthenticateAsClient`/`EndAuthenticateAsClient` pair is never
  treated as the counterpart.

- v1.52.38: 1520 tests, green locally. **CC050 (new)** —

...
A deliberately harsh health audit for the thirty-four implemented CancelCop rule IDs (CC001–CC006, CC009–CC034).
...
| CC034 | Async iterator missing a `CancellationToken` parameter | Usage | Warning | 4 | 4 | 4 | 4 | 4 | 4 | Low | **v1.35.0 (new):** flags a public/protected `IAsyncEnumerable<T>` **iterator** with no token parameter — the producer-side entry point that closes a three-way gap (CC001 sees only Task/ValueTask returns, CC011 needs a token to already exist, CC010 flags the consumer). Fixer adds `[EnumeratorCancellation]` alongside the parameter, because a bare token is silently ignored by `.WithCancellation` and would simply trade this diagnostic for CC011. Pass-through methods that return but do not yield are excluded, as are overrides, interface implementations, and `extern` (externally controlled signatures, matching CC001/CC005B). |
...
| Low | All 34 rules | Mature and FP-clean. Every rule is covered by a clean-code FP guard (`AllAnalyzersCleanCodeTests`) spanning core, framework (controllers/MediatR/SignalR/Minimal API/BackgroundService/gRPC), nested-scope, exotic-syntax, modern-C#-shape, async-File-I/O, and non-async `using` cases. Improve opportunistically. |

The rule set has grown from the original 9 (CC001–CC006, CC009) to 34 (adding CC010–CC034 across the

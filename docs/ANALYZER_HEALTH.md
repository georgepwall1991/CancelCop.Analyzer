...
Reviewed: 2026-07-21 (refreshed through the v1.27.188 hardening loop)
...
| CC015 | Blocking on async code (sync-over-async) | Usage | Warning | 4 | 4 | 4 | 4 | 3 | 4 | Low | **v1.10.0 (new); fix v1.27.188:** flags `.Result`/potentially blocking `.Wait(...)`/`.GetAwaiter().GetResult()` on a `Task`/`Task<T>`/`ValueTask` inside an `async` function; fixer rewrites safe shapes to `await`. A compile-time zero `millisecondsTimeout` is excluded because `Task.Wait(0)` is an immediate completion probe. Symbol-resolved (a look-alike `.Result` on a non-task is ignored). Shares `IsInAsyncFunction` with CC013. |

...
Reviewed: 2026-07-22 (refreshed through the v1.27.206 hardening loop)
...
| CC014 | `CancellationTokenSource` never disposed | Usage | Warning | 4 | 4 | 4 | 4 | 3 | 4 | Low | **v1.9.0 (new); fixes v1.27.179/v1.27.197/v1.27.203/v1.27.205:** flags a local `new CancellationTokenSource(...)`/`CreateLinkedTokenSource(...)` that is not a `using` decl, never disposed, and never escapes (return/out-assign/argument/nested-capture); fixer converts to a `using` declaration. Top-level-program locals use the compilation unit as their synthesized function boundary and receive the same safe `using var` fix. Compile-time-only `nameof(cts.Dispose)` does not count as disposal. Parentheses and null-forgiving operators are compile-time-only and are unwrapped before disposal/escape shape checks, so `(cts).Dispose()`, `cts!.Dispose()`, and `Use((cts!))` retain their normal semantics. Conservative escape analysis — any disposal-elsewhere path suppresses it (like a scoped CA2000 for CTS). |

...
A deliberately harsh health audit for the forty-six implemented CancelCop rule IDs (CC001–CC006, CC009–CC046).
...
| CC046 | Blocking `DbCommand.ExecuteReader` in async code | Usage | Warning | 4 | 4 | n/a | 4 | 4 | 4 | Low | **v1.49.0 (new):** flags `DbCommand.ExecuteReader` in async code (parameterless and `CommandBehavior`). CC003 is EF Core; CC045 is Open; ExecuteReader produced **zero** diagnostics from every shipped rule (empirical all-analyzers test). Symbol-gated to `System.Data.Common.DbCommand`; `ExecuteReader` is not virtual so providers' `new` hiders match by inheritance + framework shape (instance, `DbDataReader` return, 0 args or one `CommandBehavior`); custom helpers and statics stay quiet; quiet unless `ExecuteReaderAsync` exists; look-alikes, `IDbCommand`, and sync methods stay quiet. Analyzer-only. `ExecuteReaderAsync` has accepted a token since .NET Framework 4.5. `ExecuteNonQuery` / `ExecuteScalar` deferred. |
...
| Low | All 46 rules | Mature and FP-clean. Every rule is covered by a clean-code FP guard (`AllAnalyzersCleanCodeTests`) spanning core, framework (controllers/MediatR/SignalR/Minimal API/BackgroundService/gRPC), nested-scope, exotic-syntax, modern-C#-shape, async-File-I/O, and non-async `using` cases. Improve opportunistically. |

The rule set has grown from the original 9 (CC001–CC006, CC009) to 46 (adding CC010–CC046 across the
...
- **`DbCommand.ExecuteNonQuery` / `ExecuteScalar` still silent.** Empirical
  all-analyzers probe: `ExecuteNonQuery` produced 0 diagnostics (CC046 is
  ExecuteReader only). Sibling deferred.
...
- v1.49.0: 1192 tests, green locally. **new rule CC046** — blocking
  ...
  inheritance + framework shape. Custom helpers and statics stay quiet.

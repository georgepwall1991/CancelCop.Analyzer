...
- v1.52.42: 1574 tests, green locally. **CC053 (new)** —
  ...
  without a rewrite, with the speculative-rebind machinery retained so a
  future framework `JoinAsync` enables rewrites automatically. Symbol-gated
  to `System.Threading.Thread`; look-alike classes, sync methods, and other
  member names stay quiet. Overlaps CC031's family coverage of `Join`
  deliberately — CC053 is the dedicated symbol-gated rule for the type.

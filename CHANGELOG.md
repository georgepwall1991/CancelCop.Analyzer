...
- Package version synced to the release train (interim tags carried docs/test-only changes).
- Consolidated changelog backfill for the docs/test-only interim releases below.

## [1.52.50] - 2026-08-26

### Tests

- CC036 Socket arity-matrix pins completed: SendFile positive;
  ReceiveFrom/ReceiveMessageFrom withheld (parameter-modifier/result-shape mismatch).

## [1.52.49] - 2026-08-26

### Tests / Docs

- CC036 Disconnect rewrite pinned; ReceiveFrom withholding pinned; CC053 analyzer-only sample added.

## [1.52.48] - 2026-08-26

### Tests

- CC036 Accept/Connect(host, port) rewrite pins.
...
- **CC022 fixer:** null-conditional `Cancel()` statements now rewrite
  ...
  versions below C# 9 (`is not null` pattern), and calls whose
  rewritten `CancelAsync()` does not speculatively rebind to the
...
- **CC019** false negative: negated type-pattern rethrows now account for pattern polarity and
  cancellation-hierarchy overlap, so guards that still swallow `OperationCanceledException`, a
  derived cancellation type, or a possible interface-implementing subtype are diagnosed.
...
- **CC014** false positive: parenthesized `CancellationTokenSource` references now retain
  disposal and conservative escape semantics, including mixed parenthesized/null-forgiven forms.
...
- **CC015** false negative: null-conditional `Task.Wait` calls (`task?.Wait()`) are now
  diagnosed in async code. Conditional-access diagnostics intentionally have no automatic fix.
...
- Pinned the CC002 fixer on a generic overload pair: appending the token to `FooAsync(item)` binds to
  `FooAsync<T>(T, CancellationToken)` and compiles (companion to the 1.27.7 ordinal-aware analyzer
  match).

...
/// <c>Dns.GetHostEntry(...)</c>
/// to <c>await Dns.GetHostEntryAsync(..., [cancellationToken])</c>.
/// The <c>IPAddress</c> TAP is tokenless.
/// </summary>
[
    ExportCodeFixProvider(
        LanguageNames.CSharp,
        Name = nameof(BlockingDnsGetHostEntryCodeFixProvider)
    ),
    Shared
]
public class BlockingDnsGetHostEntryCodeFixProvider : CodeFixProvider
{
    private const string Title = "Use await GetHostEntryAsync";

    public sealed override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(BlockingDnsGetHostEntryAnalyzer.DiagnosticId);
    ...
    public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        if (diagnostic.Properties.ContainsKey(BlockingDnsGetHostEntryAnalyzer.NoFixProperty))
            return;
        ...
        var tokenName = diagnostic.Properties.TryGetValue(
            BlockingDnsGetHostEntryAnalyzer.TokenNameProperty,
            out var name
        )
            ? name
            : null;
        ...
        var tokenArgumentName = diagnostic.Properties.TryGetValue(
            BlockingDnsGetHostEntryAnalyzer.TokenArgumentNameProperty,
